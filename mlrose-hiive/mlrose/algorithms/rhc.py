...
@short_name('rhc')
def random_hill_climb(problem, max_attempts=10, max_iters=np.inf, max_time=np.inf, restarts=0,
                      init_state=None, curve=False, random_state=None,
                      state_fitness_callback=None, callback_user_info=None, mode_time=False, brute_time=False, full_explore=False,
                      stop_converged=False,n_iter_no_change=100,tol=0.0001, adaptive = False, min_step = 0.0001):
    ...
    base_explore_list = [i for i in range(problem.state.shape[0])]
    size_explore_list = len(base_explore_list)
    ...
    best_fitness = -np.inf
    best_state = None
    fitness_curve_modified = []
    best_score_so_far = -99
    ...
    for current_restart in range(restarts + 1):
        ...
        attempts = 0
        iters = 0
        explore_list = rd.sample(base_explore_list,size_explore_list)
        current_size_explore = size_explore_list
        while (attempts < max_attempts) and (iters < max_iters) and (time.time()-start<max_time) and current_size_explore !=0 :
            iters += 1

            if adaptive and ((stop_converged and attempts > max_attempts) or (stop_converged and iter_without_change > n_iter_no_change)):
                if(problem.step > min_step ):
                    problem.step = max(min_step, problem.step/10)
                    iter_without_change = 0
                    attempts = 0
                else:
                    break
            ...
            if(full_explore):
                temp_value = explore_list.pop(0)
                next_state = swap(problem.state,temp_value)
                current_size_explore -=1
            else:
            ...
            if curve:
                ...
                if(next_fitness>best_score_so_far):
                    best_score_so_far = next_fitness
                    best_fitness_so_far = -next_fitness
                if(stop_converged):
                    if next_fitness - best_score_so_far < tol:
                        iter_without_change +=1
                    else:
                        iter_without_change = 0
                fitness_curve_modified.append(best_fitness_so_far)
    ...
    if curve:
        if mode_time:
            return best_state, best_fitness, np.asarray(fitness_curve_modified), np.asarray(curve_time)
