...
def genetic_alg(problem, pop_size=200, pop_breed_percent=0.75, elite_dreg_ratio=0.99,
                ...
                max_attempts=10, max_iters=np.inf, max_time=np.inf, curve=False, random_state=None,
                state_fitness_callback=None, callback_user_info=None,
                hamming_factor=0.0, hamming_decay_factor=None, mode_time = False, brute_time = False,
                stop_converged=False,n_iter_no_change=100,tol=0.0001, adaptive = False, min_step = 0.0001,min_val=0, max_val=0):
    ...
    problem.min_val = min_val
    problem.max_val = max_val
    ...
    while ((attempts < max_attempts) and (iters < max_iters) and (time.time()-start<max_time)) or (brute_time and (time.time()-start<max_time)):
        ...
        if(stop_converged):
            if next_fitness - current_fitness < tol:
