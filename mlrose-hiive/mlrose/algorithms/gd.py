...
@short_name('gd')
def gradient_descent(problem, max_attempts=10, max_iters=np.inf, max_time=np.inf, 
    ...
    start = time.time()
    while (attempts < max_attempts) and (iters < max_iters) and (time.time()-start<max_time):
        iters += 1
